...
serializeJson(doc2, JsonPayload);
  ...
  if (now - lastMsg > (mqtt_settings.mqtttxintervall*1000)) {
    lastMsg = now;
    ++value;
...
thinkspeaksettings_t tssettings;
bool reloadtssettings=false;

void load_ts_settings(){
    tssettings = eepread_thinkspeaksettings();
...
void postThinkspeak( void ){
  ...
  unsigned long now = millis();
  if (now - lastMsgTS > (mqtt_settings.mqtttxintervall*1000)) {
    lastMsgTS = now;
