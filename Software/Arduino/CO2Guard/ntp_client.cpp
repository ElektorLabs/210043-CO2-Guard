...
NTP.begin ((char*)Config.ntpServerName);
 ...
 **************************************************************************************************/
void NTP_Client::SyncEvent(NTPEvent_t ntpEvent){
...
void NTP_Client::SetServerName( String ServerName ){
  ...
  NTP.setNtpServerName ( ServerName.c_str() );
}
...
void NTP_Client::Tick(){
  if(next_update>0){
    ...
    _sync=true;
    next_update = Config.SyncInterval;
  }
 ...
 *    Class         : NTP_Client
 *    Description   : Reads the NTP_Sync Interval
 *    Input         : none
 *    Output        : int32 ( in seconds )
 *    Remarks       : none
 **************************************************************************************************/
 int32_t NTP_Client::GetSyncInterval( void ){
  return (  Config.SyncInterval / 60) ;
 }
