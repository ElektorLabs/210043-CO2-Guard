...
void mqttsettings_update( ){
  ...
  if( ! server->hasArg("MQTT_TXINTERVAL") || server->arg("MQTT_TXINTERVAL") == NULL ) { 
    /* we are missong something here */
  } else { 
    uint32_t value = server->arg("MQTT_TXINTERVAL").toInt();
    Data.mqtttxinterval = value;
  }
...
void read_mqttsetting(){
  ...
  root["mqtttxinterval"] = Data.mqtttxinterval;
...
/**************************************************************************************************
 *    Function      : GetThingspeakSeverSettings
 *    Description   : response with settings to client
 *    Input         : void
 *    Output        : none
 *    Remarks       : None
 **************************************************************************************************/
void GetThingspeakSeverSettings(){
    if(server == nullptr){
        return;
    }
    thingspeaksettings_t settings = eepread_thingspeaksettings();
    String response ="";
    DynamicJsonDocument root(2048);
    root["APIKEY"]= settings.apikey;
    root["THINGSPEAK_ENA"] =(bool)(settings.ena);
    root["THINGSPEAK_TXINTERVAL"] = settings.uploadinterval;
    root["THINGSPEAK_CHID"] = settings.channelid;
    serializeJson(root, response);
...
/**************************************************************************************************
 *    Function      : SetThingspeakServerSettings
 *    Description   : Process POST with new Settings
 *    Input         : void
 *    Output        : none
 *    Remarks       : None
 **************************************************************************************************/
void SetThingspeakServerSettings( void ){
    if(server == nullptr){
        return;
    }
    ...
    /* we are missong something here */
        ...
        #ifdef DEBUG_SERIAL
         Serial.print("New ThingSpeak Key:");
         Serial.println(value);
        #endif
       
    }

    if( ! server->hasArg("THINGSPEAK_ENA") || server->arg("THINGSPEAK_ENA") == NULL ) { 
    /* we are missing something here */
    } else { 
        bool value = false;
        if(server->arg("THINGSPEAK_ENA")=="true"){
            value = true;
            #ifdef DEBUG_SERIAL
             Serial.println("ThingSpeak enable");
            #endif
        } else {
            #ifdef DEBUG_SERIAL
             Serial.println("ThingSpeak disable");
            #endif
        }
        settings.ena=value;
    }

    if( ! server->hasArg("THINGSPEAK_TXINTERVAL") || server->arg("THINGSPEAK_TXINTERVAL") == NULL ) { 
            /* we are missing something here */
    } else { 
        uint32_t value = server->arg("THINGSPEAK_TXINTERVAL").toInt();
        if( (value >=0 ) && ( value < UINT16_MAX )){
            settings.uploadinterval=value;
            #ifdef DEBUG_SERIAL
             Serial.print("New ThingSpeak TX interval:");
             Serial.println(value);
            #endif
        }
    }

    if( ! server->hasArg("THINGSPEAK_CHID") || server->arg("THINGSPEAK_CHID") == NULL ) { 
            /* we are missong something here */
    } else { 
        uint32_t value = strtoul(server->arg("THINGSPEAK_ID").c_str(),NULL,0);
        if( (value >=0 ) && ( value < UINT16_MAX )){
            settings.channelid=value;
            #ifdef DEBUG_SERIAL
             Serial.print("New ThingSpeak Channel ID:");
             Serial.println(value);
            #endif
        }
    }

    eepwrite_thingspeaksettings(settings);
